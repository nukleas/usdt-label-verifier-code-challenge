/**
 * API Route: /api/verify-legacy
 *
 * Legacy OCR processing endpoint (fallback):
 * 1. Receives form data and image
 * 2. Runs OCR on the image using basic implementation
 ...
 *
 * Processes label verification request (legacy implementation)
 */
...
/**
 * GET /api/verify-legacy
 *
 * Returns API information
 */
export async function GET() {
  return NextResponse.json({
    name: "TTB Label Verification API (Legacy)",
    version: "1.0.0",
    description: "Legacy OCR processing endpoint (fallback)",
    endpoints: {
      POST: "/api/verify-legacy - Verify label with legacy OCR",
    },
    usage: {
      method: "POST",
