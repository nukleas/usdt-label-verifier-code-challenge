/**
 * API Route: /api/verify-server
 *
 * Server-side OCR processing endpoint:
 * 1. Receives form data and OCR results from client
 * 2. OR processes image directly with server-side OCR
 ...
 *
 * Processes label verification with server-side OCR
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      // Convert File to Buffer for server-side OCR
      ...
      console.log(
        `Server-side OCR completed. Confidence: ${ocrResult.confidence}%`
      );
    ...
    console.log(
      `Server-side verification completed in ${verificationResult.processingTime}ms`
    );
...
/**
 * GET /api/verify-server
 *
 * Returns API information
 */
export async function GET() {
  return NextResponse.json({
    name: "TTB Label Verification API (Server-Side OCR)",
    version: "1.0.0",
    description:
      "Server-side OCR processing for better performance and reliability",
    endpoints: {
      POST: "/api/verify-server - Verify label with server-side OCR",
    },
    usage: {
      method: "POST",
